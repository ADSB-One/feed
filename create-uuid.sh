...
if [ -f /boot/adsb-config.txt ]; then
    UUID_FILE="/boot/adsb-one-uuid"
else
    mkdir -p /usr/local/share/adsb-one
    UUID_FILE="/usr/local/share/adsb-one/adsb-one-uuid"
    # move old file position
    if [ -f /boot/adsb-one-uuid ]; then
        mv -f /boot/adsb-one-uuid $UUID_FILE
    fi
fi
